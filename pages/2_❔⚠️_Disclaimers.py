...
st.divider()
st.warning("The purpose of this chatbot is to answer questions about psychology with greater technical fidelity by making use of the DSM-5. To this end, and to avoid the rabbit holes AI sometimes falls down into over the course of longer, iterative conversations, all of the available 'memory' of the chatbot was dedicated to referencing the DSM-5. As a result, this chatbot intentionally CANNOT remember your prior questions or its own prior responses. If you want to tweak your output, try asking your question in a different way. Prompts such as, 'Restate your previous answer as if you had a target audience of 7th graders,' or 'Can you give me that information in a table?' will NOT work properly." , icon="⚠️")
st.divider()
st.warning("Note that if you ask the AI something like, 'That last answer was a bit over my head. Can you dumb that down for me?', the AI will still search the DSM-5 for passages similar to your query, and probably find excerpts on topics such as learning disabilities or cognition inventories given the language used in the query. Thus, you'll still get a response, but it will be strange and disjointed, and the context to the response will be non-obvious. So don't ask questions that depend on conversational memory, as you'll only get worthless answers. I thought about specifically forbidding the AI to answer questions that seemed to reference memory, but, well, that seemed self-evidently problematic given the context.", icon="⚠️")
st.divider()
